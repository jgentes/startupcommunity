...
var getlinkedinprofile = function(url, email, access_token, profilecallback) {       
  ...
  function(error, response, body) {
    if (!body.status || body.status === 200) {
      ...
      console.warn('Linkedin profile error: ' + body.message);
    ...
    // Step 2. Retrieve profile information about the current user.
      ...
      db.newSearchBuilder()
        ...
        for (var i=0; i < userlist.length; i++) {          
          getlinkedinprofile(userlist[i].url, userlist[i].email, params.oauth2_access_token, function(response) { return response;});
        }        
