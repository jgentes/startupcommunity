  ...
  .factory('userService', function($http) {
    return {        
      search: function(city, query) {
        return $http.get('/api/1/' + city + '/users' + (query ? '?search=' + query : ''));
      },
      getUsers: function(city, cluster, role, limit, alturl) {
        return $http.get(alturl || '/api/1/' + city + '/users' + (cluster ? '?cluster=' + cluster : '') + (cluster && role ? '&' : '?') + (role ? 'role=' + role : '') + (limit ? (cluster || role) ? '&limit=' + limit : '?limit=' + limit : ''));
      },
      putUser: function(userid, profile, callback) {
        $http.put('/api/1/user/' + userid + '?profile=' + profile)
      ...
      getProfile: function(userid) {        
        return $http.get(userid ? '/api/1/profile/' + userid : '/api/1/profile'); // return me if no userid is provided
      },
      putProfile: function(profileData) { // addcallback!
        return $http.put('/api/1/profile', profileData);
      },
      removeProfile: function(userid, callback) {
        $http.post('/api/1/profile/remove/' + userid)
        .success( function(response) {
          callback(response);
        })
        .error( function(response) {
      ...
      addPerson: function(url, email, userid, callback) {        
        $http.get('/api/1/addPerson?user={"url":"' + url + '","email":"' + email + '","userid":"' + userid + '"}')
        .success( function(response) {
          callback(response);
        })
        .error( function(response) {
      ...
      getKey: function() {
        return $http.get('/api/1/profile/getkey');
      },      
      setCityAdvisor: function(userkey, citykey, role, status, callback) {
        $http.put('/api/1/profile/role?userkey=' + userkey + '&citykey=' + citykey + '&role=' + role + '&status=' + status)
      ...
      setRole: function(userkey, citykey, cluster, role, status, callback) {
        $http.put('/api/1/profile/role?userkey=' + userkey + '&citykey=' + citykey + '&cluster=' + cluster + '&role=' + role + '&status=' + status)
        .success( function(data, status) {
          callback(data, status);
        })
        .error( function(data, status) {
  ...
  .factory('cityService', function($http) {
    return {
      getCity: function(city) {
        return $http.get('/api/1/city/' + city);
      }
    };
  })
