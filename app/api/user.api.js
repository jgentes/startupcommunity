...
function handleDirectSearch(req, res) {
    ...
    cdb.find({selector: {type: 'user', '$text': req.query.query}, skip: Number(req.query.offset) || 0, limit: Number(req.query.limit) || 100}, function(err, result) {
      if (!err) {
        result = formatFindResults(result);
...
function handleContactUser(req, res) {
  ...
  cdb.search('communities', 'communitySearch', {q: searchstring, include_docs: true}, function(err, result) {
    if (!err) {
